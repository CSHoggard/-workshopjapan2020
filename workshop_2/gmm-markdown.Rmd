...
This practical constitutes the second workshop of the #StayHomebutStudy event, organised by Atsushi Noguchi, Dr. Christian Steven Hoggard and Prof. Ben Marwick and is tailored for Japanese archaeologists, researchers and enthusiasts.  
...
このガイドで使用されるデータは，Ivanovaitė et al (2020)「これらは素晴らしい文化ですか? 東ヨーロッパ後期旧石器時代の剥片尖頭器文化をめぐる研究史と地域性」『ヨーロッパ考古学』で公表されたものである(https://doi.org/10.1017/eaa.2019.59)．同論文のオリジナルデータはGitHubリポジトリ、(https://github.com/CSHoggard/-Eastern-Europe-Tanged-Points)およびOSFリポジトリ(https://osf.io/agrwb/)でも公開されている。
...
この実習で用いるすべてのコード，データ，マークダウン文書（HTMLとPDFフォーマットを含む）は、GitHub上のワークショップのリポジトリで公開されている(https://github.com/CSHoggard/-japanworkshop2020tree/master/workshop_2)．
...
ここで実行するGMMの方法は，2Dのアウトライン分析である．下記の2つのパッケージ（および付随してインポートされるパッケージ群）を必要とする．    
...
For literature pertaining to outline analysis see（参考文献）:\
...
```{r}
database <- rio::import("https://github.com/CSHoggard/-workshopjapan2020/raw/master/workshop_2/database.rds") # read in from GitHub／GitHubからの読み込み

tpsdata <- rio::import("https://github.com/CSHoggard/-workshopjapan2020/raw/master/workshop_2/tpslines.rds") # read in from GitHub／GitHubからの読み込み
```
...
tidyverseとMomocsパッケージの読み込みは，ファイルサイズの大きさから多少時間がかかるかも知れません．少し待ってください．一度インストールされたパッケージは，`library()` 関数によってアクティベートされます：    
...
このデータは，東ヨーロッパの晩期旧石器時代における文化的分類のロバストネス（頑健性）を，剥片尖頭器のバリエーションから評価するために作成されました．250点の剥片尖頭器のアウトラインデータで，TPS Suite(https://life.bio.sunysb.edu/morph/soft-dataacq.html)の*outline object* 関数を使用して作成されました．ファイルフォーマットは *.tps* です．アウトラインは，アルゴリズムによって生成された等間隔の点群セミランドマークとして構成されています．すべての資料とその考古学的分化の帰属もデータベース化されています．
