---
title: '**Archaeological Geometric Morphometrics and R／考古学における幾何形態測定学とR**'
author: c("Dr Christian Steven Hoggard (University of Southampton, United Kingdom)", "Atsushi Noguchi (translator)")
...
## **Importing GMM Data: Alternative Approaches／GMMデータの読み込み：ほかの方法**  
...
<<<<<<< HEAD
形態測定学のためのランドマークおよびアウトラインデータをRに読み込む方法はいくつもあります．ここでは簡便さと再現性のために，GitHubリポジトリに保存した.tps形式のアウトラインデータを `Momocs::import_tps()` 関数を使用して直接読み込んでいます．ローカルに保存した.tpsデータを，上記の関数，`geomorph::readland.tps()` 関数で読み込み,`Momocs::rw_rule()` 関数で書き換える方法もあります．stereomorphで作成したデータは，`Momocs::import_StereoMorph_ldk()` および `Momocs:import_StereoMorph_curve()` 関数で読み込みことができます．\    
...
=======
Note: for the purpose of this workshop I will detail in-text the function and its constitutent package e.g. `geomorph::readland.tps()`, however only the function is what will be 'used' so-to-speak e.g. `readland.tps()`. This helps you to understand what packages the functions originate from.  
>>>>>>> 3df56d3fb9c3229dd69d6a2939f4d6a310efcd9a

注：このワークショップでは，説明文中では，関数を，それが含まれるパッケージと一緒に記載しています．例)`geomorph::readland.tps()` では，`geomorph` がパッケージ名で，`readland.tps()` が関数名．これは，各関数がどのパッケージによるものなのかを分かりやすくするためです．実際には関数名だけで実行できます．\    
...
Momocsでは，塗りつぶしたシルエット画像データから，`Momocs::import_jpg()` および `Momocs::import_jpg1()` 関数を用いてアウトラインを取得することもできます．パッケージ・ペインの `Momocs` から，これらの関数の詳細を見ることができます．またこの手法については，ワークショップの最後に実演します．\    
...
## **GMM Procedure 7: Multiple Analysis of Variance (MANOVA)／GMMの実行7：多変量分散分析(MANOVA)**
